...
@Deprecated
public class RequestFriendsFromVk {
    ...
    /*public RequestFriendsFromVk(HttpClient client, PostMethod initialMethod) {
        this.client = client;
    ...
    private static List<User> parseJSON(String json) {
        ...
        for (List<String> friend : (List<List<String>>) friends) {
            ...
            //result.add(new UserVo(id, pic, page, isFemale ? UserVo.Gender.FEMALE : UserVo.Gender.MALE, name));
        }
        return result;
    }
    ...
    public void run() {
        try {
            ...
            *//*
            ...
            List<ru.friends.UserVo> friends = new ArrayList<ru.friends.UserVo>();
            Pattern pattern = Pattern.compile("','");
            for(String line : lines) {
                String[] fields = pattern.split(line);
                friends.add(new ru.friends.UserVo(Integer.parseInt(fields[0]),fields[1],fields[2],"1".equals(fields[3]) ? ru.friends.UserVo.Gender.FEMALE : ru.friends.UserVo.Gender.MALE,fields[5]));
            }
            *//*
            ...
            //final List<UserVo> oldFriends = jdbc.executeUsers("SELECT * FROM friends WHERE(del_date IS NULL)");
